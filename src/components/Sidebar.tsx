...
const Sidebar: React.FC<SidebarProps> = ({ className = '', activePage, onPageChange, collapsed = false, onToggleCollapse }) => {
  ...
  return (
    <div className={`sidebar flex flex-col h-full bg-base-200 border-r border-base-300 transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'} ${className}`}>
      {/* 用户信息 */}
      <div className={`${collapsed ? 'p-3' : 'p-4'} border-b border-base-300`}>
        <div className={`flex items-center ${collapsed ? 'justify-center' : 'space-x-3'}`}>
          <div className="avatar cursor-pointer" onClick={onToggleCollapse}>
            <div className={`${collapsed ? 'w-10 h-10' : 'w-12 h-12'} rounded-full`}>
              {user?.avatar_url ? (
                <img 
                  src={user.avatar_url} 
                  alt={user.username}
                ...
                <div className={`bg-primary/20 flex items-center justify-center w-full h-full text-primary font-semibold ${collapsed ? 'text-base' : 'text-lg'}`}>
      ...
      {/* 导航菜单 */}
      <div className={`flex-1 ${collapsed ? 'px-2 py-4' : 'px-3 py-4'}`}>
        <ul className="menu menu-vertical w-full space-y-2">
          {menuItems.map((item) => (
            <li key={item.id}>
              <a 
                className={`flex items-center ${collapsed ? 'justify-center p-3' : 'p-3'} rounded-lg cursor-pointer ${
                  item.active 
              ...
              >
                ...
                {item.badge > 0 && (
                  <div className={`badge badge-sm ${collapsed ? 'absolute -top-1 -right-1' : 'ml-auto'} ${
      ...
      {/* 底部操作 */}
      <div className={`${collapsed ? 'p-3' : 'p-4'} border-t border-base-300`}>
        <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
          ...
          <label className="swap swap-rotate cursor-pointer">
            ...
            />
            <div className="swap-off p-2 rounded-lg hover:bg-base-300">
              <svg className="w-5 h-5 text-warning" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" clipRule="evenodd" />
              </svg>
            </div>
            <div className="swap-on p-2 rounded-lg hover:bg-base-300">
